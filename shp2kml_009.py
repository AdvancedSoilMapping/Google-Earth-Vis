...
def main():
    print("\n--- EM Survey Accurate-Projection Generator ---\n")
    ...
    try:
        ...
        gdf.set_crs(epsg=epsg, allow_override=True, inplace=True) # Define original Projection
    ...
    # 3. REPROJECT TO WGS84 (THE CRITICAL FIX)
    # We convert to Lat/Long *before* grid generation to eliminate distortion/rotation issues
    print("Reprojecting to WGS84 to fix distortion...")
    gdf = gdf.to_crs(epsg=4326)
    bound_gdf = bound_gdf.to_crs(epsg=4326)

    # 4. Create Interpolated Grid in Lat/Long
    ...
    # 1 degree lon is approx 111,320 * cos(lat) meters
    mean_lat = np.mean(y)
    ...
    pad_lon = res_lon * 2
    pad_lat = res_lat * 2
    x_min -= pad_lon; x_max += pad_lon
    y_min -= pad_lat; y_max += pad_lat
    ...
    im = ax.imshow(grid_z, cmap=cmap, vmin=min_cut, vmax=max_cut, 
                   ...
                   # aspect='auto' ensures it stretches correctly to fill the lat/lon box
    
    # Apply Clip
    ax.add_patch(clip_patch)
    im.set_clip_path(clip_patch)
    
    # Plot Boundary (Use linewidth scaled relative to figure size roughly, or trial/error)
    # Since we are in degrees now, standard linewidths might look different.
    ...
    # Bounds are now Native WGS84 because we reprojected earlier
    ...
    kmz_output = base_path + "_Accurate.kmz"
