...
# --- CONFIGURATION ---
DEFAULT_COLUMN = "EM" 
DEFAULT_PIXEL_SIZE = 5.0  # Meters (We will convert this to degrees automatically)
...
def main():
    print("\n--- EM Survey Native WGS84 Generator ---\n")
    ...
    # 2. Load & Reproject IMMEDIATELY to WGS84
    print("Reading Data & Converting to WGS84...")
    try:
    ...
    print("Calculating Grid (Degrees)...")
    x = gdf.geometry.x.values # Longitude
    y = gdf.geometry.y.values # Latitude
    z = gdf[col_name].values

    # Convert resolution (meters) to degrees based on mean Latitude
    mean_lat = np.mean(y)
    ...
    # Pad the grid by 2 pixels so the boundary line doesn't get cut off
    pad_x = res_lon * 2
    pad_y = res_lat * 2
    ...
    # 4. Interpolate
    # We use linear interpolation in Lat/Long space
    print("Interpolating...")
    ...
    # 'aspect=auto' allows Matplotlib to stretch the degrees to fill the square figure
    # 'extent' locks the image to the WGS84 coordinates
    im = ax.imshow(grid_z, cmap=cmap, vmin=min_cut, vmax=max_cut, 
                   origin='lower', interpolation='bilinear',
                   extent=[x_min, x_max, y_min, y_max], aspect='auto')
    
    # Apply the vector clip
    ax.add_patch(clip_patch)
    im.set_clip_path(clip_patch)
    
    # Draw the boundary line
    # Note: 'linewidth' might behave differently in degree-space vs screen-space.
    # We stick to a standard value here.
    ...
    # 7. Create KML (Standard LatLonBox)
    ...
    # Since we generated the image in WGS84, standard mapping works perfectly
    ...
    kmz_output = base_path + "_Native.kmz"
