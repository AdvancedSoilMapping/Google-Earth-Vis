...
def main():
    print("\n--- EM Survey Final Generator (Smooth + Anti-Skew) ---\n")
    ...
    # 4. Vector Clip (Smooth Edges)
    print("Applying Smooth Clipping...")
    ...
    # Save at 150 DPI (Good balance)
    plt.savefig(img_output, dpi=150, transparent=True, pad_inches=0)
    plt.close()

    # 6. Calculate Anti-Skew Coordinates
    print("Calculating Rotated Corner Points...")
    
    # We define the 4 corners of the MGA image
    corners_mga = [
    ...
    # Convert these exact corners to WGS84 Lat/Long
    corners_gdf = gpd.GeoSeries(corners_mga, crs=epsg)
    corners_wgs84 = corners_gdf.to_crs(epsg=4326)
    
    # Format for SimpleKML (Long, Lat)
    # The order must be: BL, BR, TR, TL
    coords = [
    ...
    # 7. Create KML with "Quad" (Warped Placement)
    ...
    # This is the magic command that fixes the skew
    ground.gxlatlonquad.coords = coords
    
    ground.color = "ffffffff" # Solid opacity

    kmz_output = base_path + "_Final.kmz"
